...
track_init, track_terminate = 2, 17  # track starts appearing, disappears
...
x_starts = center[0] + radius * np.cos(theta)
y_starts = center[1] + radius * np.sin(theta)
dx = center[0] - x_starts
dy = center[1] - y_starts
...
for mc_s in range(MC_simulation):

    Zk = []
    GT = []

    gt = np.array([x_starts[mc_s], y_starts[mc_s], dx[mc_s], dy[mc_s]]).reshape(-1, 1)
    ...
    MC_ZK.append(Zk)
    MC_GT.append(GT)
    simulation_records['Zk_' + str(mc_s)] = Zk
    simulation_records['GT_' + str(mc_s)] = GT

"""for mm in MC_GT:
    for sim in mm:
        if len(sim) == 0:
            continue
        plt.scatter(sim[0], sim[1], c='r')
...
def reset():
    ...
    plt.savefig('bernoulli_pf_parametre_1/initialization.png', dpi=1100, bbox_inches='tight')
    plt.clf()
...
for monte_carlo_simulation_i in range(MC_simulation):
    ...
    print("** simulation: {}".format(monte_carlo_simulation_i))

    for i in range(n_simulation):

        Xk_km1, wk_km1, qk_km1 = predict(qkm1_km1=qk_k, Xkk=Xk_k, wkk=wk_k, dt=1, N_adaptive_birth=N_adaptive_birth)

        wk_k, Xk_k, qk_k, lk_array, N_adaptive_birth = update(Zk=MC_ZK[monte_carlo_simulation_i][i], Xk_km1=Xk_km1, wk_km1=wk_km1, qk_km1=qk_km1)
        ...
        ospa_dist_MMSE = OSPA.compute_OSPA_distance(track_states=list(MMSE[:2].flatten()), truth_states=MC_GT[monte_carlo_simulation_i][i])

        ospa_dist_MAP = OSPA.compute_OSPA_distance(track_states=list(MAP[:2].flatten()), truth_states=MC_GT[monte_carlo_simulation_i][i])
        ...
        for zk_i in MC_ZK[monte_carlo_simulation_i][i]:
            plt.scatter(zk_i[0], zk_i[1], c='red')
        plt.scatter(MC_ZK[monte_carlo_simulation_i][i][-1][0], MC_ZK[monte_carlo_simulation_i][i][-1][1], c='red', label='detections')
        ...
        plt.legend()
        #plt.savefig('bernoulli_pf_parametre_1/bernoulli_particle_filter_spatial_' + str(i) + '.png', dpi=1100, bbox_inches='tight')
        plt.pause(0.01)"""

    simulation_records['qk_' + str(monte_carlo_simulation_i)] = qk_records
    simulation_records['map_' + str(monte_carlo_simulation_i)] = map_records
    simulation_records['ospa_MAP_records_' + str(monte_carlo_simulation_i)] = ospa_MAP_records

with open('bpf_monte_carlo_simulation_5.json', 'w') as json_file:
